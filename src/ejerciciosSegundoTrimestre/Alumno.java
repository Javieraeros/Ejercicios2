...
public class Alumno implements Cloneable,Comparable<Alumno>{
	 ...
	 * 0 si son iguales, -1 en caso contrario. Un Alumno es mayor que otro si la primera, segunda o sucesivas letras de su nombre
	 * son anteriores a las letras de otro Alumno. En caso de que tengan el mismo nombre,se comparan por apellidos 
	 * y si tienen los mismos apellidos se compararan por notas, siendo la de mayor calificación mayor que la otra.
	 */
	...
	public int compareTo(Alumno comparado){
		...
		if (this.nombre.length()<comparado.getNombre().length()){
			tamanyo=this.nombre.length();							//No es necesario el -1 porque en el for pongo < y NO <=
		}else{
		...
		if(resul==0){
			...
			if(nota>comparado.getNota() && resul==0){
				resul=1;
			}else{
				if(nota<comparado.getNota()){
					resul=-1;
				}
			}
		}
